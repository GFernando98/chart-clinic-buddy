...
export const UserFormDialog = ({ open, onOpenChange, user, onSave, isSaving = false }: UserFormDialogProps) => {
  ...
  useEffect(() => {
    if (open) {
      if (user) {
        ...
        form.reset({ userName: '', firstName: '', lastName: '', email: '', password: '', roles: [] });
      }
    }
  }, [open, user, form]);
  ...
  const onSubmit = async (values: UserFormValues) => {
    ...
    if (user) { userData.id = user.id; userData.isActive = user.isActive; }
    else { userData.id = crypto.randomUUID(); userData.isActive = true; }
    await onSave(userData);
  };

  const roleLabels: Record<UserRole, string> = {
    Admin: t('roles.admin'), Doctor: t('roles.doctor'),
    Receptionist: t('roles.receptionist'), Assistant: t('roles.assistant'),
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{isEditing ? t('users.editUser') : t('users.newUser')}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="firstName" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder={t('patients.firstNamePlaceholder')} {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="lastName" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder={t('patients.lastNamePlaceholder')} {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
                        ...
                        <FormControl>
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              ...
                              field.onChange(checked ? [...current, role] : current.filter((r) => r !== role));
                            }}
                          />
                        </FormControl>
                        <FormLabel className="font-normal cursor-pointer">{roleLabels[role]}</FormLabel>
                      </FormItem>
                    )} />
                  ))}
                </div>
